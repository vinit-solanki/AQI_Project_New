...
# Page config
st.set_page_config(
    page_title="Weekly PM2.5 Predictions",
    page_icon="📈",
    layout="wide"
)
...
# Main page
st.title("Weekly PM2.5 Predictions")
...
# Train models
train_data = pm25_series[:-7]  # Use all but last week for training
arima_model, residual_model = train_models(train_data)

# Generate weekly predictions
weekly_pred = generate_predictions(arima_model, residual_model, train_data, periods=7)

# Display predictions
st.subheader("Next Week's PM2.5 Forecasts")
col1, col2 = st.columns(2)

with col1:
    # Plot predictions
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(pm25_series[-14:], label='Historical', color='blue')
    ax.plot(weekly_pred, label='Forecast', color='red')
    ax.fill_between(
        weekly_pred.index,
        weekly_pred * 0.9,
        weekly_pred * 1.1,
    ...
    ax.set_title("PM2.5 Weekly Forecast")
    ax.set_xlabel("Date")
...
with col2:
    ...
    pred_df = pd.DataFrame({
        'Date': weekly_pred.index.strftime('%Y-%m-%d'),
        'PM2.5 Forecast': weekly_pred.round(2),
        'Lower Bound': (weekly_pred * 0.9).round(2),
        'Upper Bound': (weekly_pred * 1.1).round(2)
    })
    st.dataframe(pred_df)

# Download predictions
st.download_button(
    label="Download Weekly Predictions",
    data=pred_df.to_csv(index=False),
    file_name="weekly_pm25_predictions.csv",
    mime="text/csv"
)
...
recent_actual = pm25_series[-30:-7]  # Last 30 days excluding prediction period
recent_pred = generate_predictions(arima_model, residual_model, pm25_series[:-30], periods=23)
